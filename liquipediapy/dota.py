  ...
  def get_upcoming_and_ongoing_games(self):
    ...
    for match in matches:
      ...
      try:
        ...
        game['tournament_short_name'] = cells[3].find(
          'div').get_text().rstrip()
        try:
    ...
    for row in div_rows:
      ...
      try:
        tournament['icon'] = self.__image_base_url + row.find(
          'div', class_="divCell Tournament Header").find('img').get('src')
      except AttributeError:
        pass
      ...
      tournament['dates'] = row.find(
        'div', class_="divCell EventDetails Date Header").get_text().strip()

      try:
        tournament['prize_pool'] = int(
          row.find('div', class_="divCell EventDetails Prize Header").get_text(
      ...
      tournament['teams'] = re.sub(
        '[A-Za-z]', '',
        row.find('div', class_="divCell EventDetails PlayerNumber Header").
        get_text()).rstrip()
      ...
      if winner:
        ...
        tournament['runner_up'] = row.find(
          'div', class_="divCell Placement SecondPlace").get_text().strip()
      else:
