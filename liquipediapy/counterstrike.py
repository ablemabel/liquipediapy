...
class counterstrike():
  ...
  def get_players(self):
    regions = [
      'Europe', 'CIS', 'Americas', 'Oceania', 'Eastern_&_Southern_Asia',
      'Africa_&_Middle_East'
    ]
    players = []
    for region in regions:
      ...
      for row in rows:
        ...
        try:
          team = row.find(
            'span', class_='team-template-image-icon').find('a').get('title')
  ...
  def get_teams(self, region):
    ...
    for table in tables:
      ...
      header = rows[0]
      team['name'] = header.find('span',
                                 class_='team-template-text').get_text()
      team['logo'] = self.__image_base_url + header.find(
        'span', class_='team-template-team-standard').find('img').get('src')
      team['playes'] = []
      rows = rows[2:]
      for row in rows:
        player = {}
    ...
    for match in matches:
      ...
      try:
        game['team1'] = cells[0].find(
          'span', class_='team-template-image').find('a').get('title')
  ...
  def get_tournaments(self, tournamentType=None):
    ...
    for table in tables:
      ...
      for row in rows:
        ...
        tournament['prize'] = unicodedata.normalize(
          "NFKD",
          row.find('div', class_='Prize').get_text().rstrip())
        team_no = unicodedata.normalize(
          "NFKD",
          row.find('div', class_='PlayerNumber').get_text().rstrip()).split()
        tournament['teams_no'] = team_no[0]
        location = unicodedata.normalize(
          "NFKD",
          row.find('div', class_='Location').get_text().rstrip()).split(',')
        try:
        ...
        if len(placement) > 1:
          try:
            tournament['first_place'] = row.find(
              'div', class_='FirstPlace').find(
                'span', class_='team-template-text').get_text()
          except AttributeError:
            tournament['first_place'] = 'TBD'

          try:
            tournament['second_place'] = row.find(
              'div', class_='SecondPlace').find(
                'span', class_='team-template-text').get_text()
          except AttributeError:
            tournament['second_place'] = 'TBD'
        else:
          teams = row.find('div', class_='Placement').find_all('span',
                                                               class_='Player')
          qual_teams = []
          for team in teams:
            qual_teams.append(
              team.find('span', class_='team-template-text').get_text())
